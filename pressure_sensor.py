...
class Pressuresensor(BaseSensor):
    def __init__(self, message_callback):
        super().__init__("Pressure_sensor", 0x125, message_callback, default_value=101325)
    ...
    def setup_controls(self, frame):
        ttk.Label(frame, text="Total Pressure:", font=('Arial', 12)).grid(row=0, column=0, padx=5, pady=5)
        self.value_label = ttk.Label(frame, textvariable=self.value, font=('Arial', 12))
        self.value_label.grid(row=0, column=1, padx=5, pady=5)
        ttk.Label(frame, text="Pa", font=('Arial', 12)).grid(row=0, column=2, padx=5, pady=5)

        self.slider = ttk.Scale(frame, from_=0, to=1000, orient=tk.HORIZONTAL,
